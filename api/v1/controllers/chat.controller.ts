...
export const sendFileMessage = async (
  ...
  try {
    ...
    await RoomChat.findByIdAndUpdate(room_chat_id, {
...
export const reactMessage = async (
  ...
  try {
    ...
    const existingReactIndex = chat.reactions.findIndex(
      (r) => r.user_id.toString() === currentUserId,
    );
...
export const deleteMultipleMessages = async (
  req: Request,
  res: Response,
): Promise<any> => {
  try {
    ...
    if (req.user.role !== "ADMIN") {
      const unauthorized = chats.filter(
